...
//        String sql = "insert into " + Constant.product_sql + "(id,title,price,category," + "itemIds,modifyTime,tid,isChanged,weight,virtualWeight) values(?,?,?,?,?,?,?,?,?,?)";
//        try {
...
//        int count = 0;
//        try {
//            String sql = "select * from  " + Constant.product_sql + " where itemIds like '%" + product.pid + "%'";
...
//                    ischange = "1";
//                }
//                if ((!rss.getString("itemIds").equals(product.itemIds)) && (product.itemIds != null)) {
//                    updateSingleValue(id, "itemIds", product.itemIds);
//                    ischange = "1";
//                }
...
//            if (rs.next()) {
//                String sql2 = "update " + Constant.product_sql + " set tid=? where itemIds like  '%" + product.pid + "%' ";
//                PreparedStatement pstmt = SQLUtils.getConnection().prepareStatement(sql2);
...
//
//        p.setCategory(IkeaUtils.getCategory(buf));
//        p.itemTypes = IkeaUtils.getItemTypes(buf);
//
...
//     * 添加一个完整的产品到数据库
//     */
//    public static void addToSQL2(Product p, int itemToggle) {
//        sendProductToSQL(p);
//        if (itemToggle == 1)
//            if (p.itemsList.size() != 0)
//                for (Item item : p.itemsList) {
//                    if(DaoFactory.getItemDao().check(item)==0)
//                    DaoFactory.getItemDao().insertItem(item);
//                    else
//                    System.out.println(item.getPid()+"已存在");
//                }
//            else
//                System.err.println("itemsList为空");
//    }
...
//        Product p = new Product(id);
//
//        String sql = "select * from  " + Constant.product_sql + " where itemIds like '%" + id + "%'";
//        try {
//            Statement stmt = SQLUtils.getConnection().createStatement();
//            ResultSet rs = null;
//            rs = stmt.executeQuery(sql);
...
//                p.itemIds = rs.getString("itemIds");
...
//            if (rs.next()) {
//                String itemListString = rs.getString("itemIds").replace("[", "").replace("]", "").replace(" ", "");
